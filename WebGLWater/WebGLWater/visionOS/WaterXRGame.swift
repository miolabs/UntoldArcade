...
final class WaterXRGame {
    ...
    func start() {
        enableWater(true)
        setWaterSphere(center: ballLocal, radius: radius)
        setWaterLightDirection(simd_float3(2.0, 2.0, -1.0))
        loadArt()
        seedWaterRipples(count: 8)
        updateModel()
    ...
    private func updateModel() { setWaterModelMatrix(model()) }
    ...
    func update(deltaTime: Float) {
        ...
        setWaterSphereCenter(ballLocal)
        updateModel()
        setWaterOcclusionMeshes(SceneOcclusion.shared.currentMeshes())
    }
