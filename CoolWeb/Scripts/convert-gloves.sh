...
# Outputs:  Examples/CoolWebVisionOS/.../Resources/Models/techglove_{right,left}.usdz
...
"$blender" --background "$src/hand_R.blend" --python "$script_dir/convert_hand.py" \
    -- Right "$out/techglove_right.usdz" "$package_root/Sources/CoolWeb/Resources/Models/textures"
"$blender" --background "$src/hand_L.blend" --python "$script_dir/convert_hand.py" \
    -- Left "$out/techglove_left.usdz" "$package_root/Sources/CoolWeb/Resources/Models/textures"
...
for side in right left; do
    for tag in preview palm; do
        mv -f "$out/techglove_${side}_${tag}.png" \
