...
A *virtual mirror* for the engine's character-deformation stack: a rigged, high-poly character stands in front of you flexing, and you switch the skinning live — legacy vertex shader, compute LBS, dual-quaternion, or Direct Delta Mush — freeze the pose to compare, drag morph sliders, or let pose drivers fire the biceps automatically as the elbows curl. Switch on the muscle simulation and a full-body set of volumetric XPBD muscles (built procedurally from the rig, no extra assets) bulges under the skin as the joints bend, with a "flex all" slider for the pose-off shot, a wireframe overlay of the cages and per-muscle controls. Or pick the ML deformer: a small network trained on that simulation (`swift run CoolMirrorBake` + the engine's `train_mldeformer.py`, the `.untoldml` ships next to each hero) that predicts the same deltas from the pose in one dispatch, with a blend slider to A/B it. And the mirror part: put an iPhone on a stand facing you, run the bundled **CoolMirror Capture** app (`CoolMirror/Examples/CoolMirrorCapture`, rear-camera ARKit body tracking → Bonjour/UDP; the phone stands sideways, body tracking is landscape-only), tap "Use iPhone body tracking" on the Vision Pro and follow the steps it shows (the phone's screen faces away from you, so the headset guides the setup): step into the phone's view, stand like the character, hold the pose for the 3-second calibration, and the character mirrors your body live — with any of the deformation modes on top.

```bash
open CoolMirror/Examples/CoolMirrorVisionOS/CoolMirrorVisionOS.xcodeproj
open CoolMirror/Examples/CoolMirrorCapture/CoolMirrorCapture.xcodeproj   # iPhone capture app
```
