//
//  CoolBallJoltSimulationTests.swift
//  CoolBallTests
//
...
final class CoolBallJoltSimulationTests: XCTestCase {
    func testFloorPlaneBecomesASlabUnderTheSurface() {
        let floor = CoolBallWorldPlane.infiniteFloor(y: 0.3)
        let box = CoolBallJoltSimulation.environmentBox(for: floor)
        XCTAssertEqual(box.center.y, 0.3 - CoolBallJoltSimulation.slabHalfThickness, accuracy: 1e-6)
        XCTAssertEqual(box.halfExtents.z, CoolBallJoltSimulation.slabHalfThickness, accuracy: 1e-6)
        XCTAssertEqual(box.halfExtents.x, CoolBallJoltSimulation.maxHalfExtent, "Infinite extents are capped")
    ...
    func testWallPlaneKeepsItsOrientation() {
        ...
        let box = CoolBallJoltSimulation.environmentBox(for: wall)
        XCTAssertEqual(box.halfExtents.x, 1.5, accuracy: 1e-6)
        XCTAssertEqual(box.halfExtents.y, 1.0, accuracy: 1e-6)
        XCTAssertEqual(box.center.z, -2 - CoolBallJoltSimulation.slabHalfThickness, accuracy: 1e-6)
    ...
    func testBallRestsOnADetectedFloorThroughJolt() {
        ...
        let simulation = CoolBallJoltSimulation(backend: jolt)
        ...
        XCTAssertEqual(state.position.y, -1.0 + CoolBallScene.ballRadius, accuracy: 0.02, "Rests on the slab's top face")
