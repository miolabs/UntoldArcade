...
public final class CoolBallAudio: @unchecked Sendable {
    ...
    /// The bounce thump. `intensity` 0…1 scales volume; rapid contacts are
    /// rate-limited so a rolling ball doesn't machine-gun.
    public func playBounce(intensity: Float) {
        let now = ProcessInfo.processInfo.systemUptime
        stateLock.withLock {
            guard now - lastBounceTime > 0.09 else { return }
            lastBounceTime = now
        }
        enqueueOneShot(bank: Bank.bounce, volume: min(max(intensity, 0.12), 1.0))
    }

    public func playScore() {
        enqueueOneShot(bank: Bank.score, volume: 0.85)
    }
    ...
    private func startOnQueue() {
        ...
        if sampleBanks.isEmpty {
            sampleBanks = [makeBounce(), makeScore()]
        }
    ...
    /// The bounce: a punchy low thump — a pitch-dropping sine (130 → 42 Hz)
    /// under a short leather-slap noise click, both exponentially decayed.
    private func makeBounce() -> [Float] {
        let duration = 0.22
    ...
    /// The score: a stadium-horn chord (three detuned voices with vibrato
    /// and odd harmonics) over a crowd-roar swell of shaped noise.
    private func makeScore() -> [Float] {
        let duration = 2.0
        let count = Int(duration * sampleRate)
        var samples = [Float](repeating: 0, count: count)
