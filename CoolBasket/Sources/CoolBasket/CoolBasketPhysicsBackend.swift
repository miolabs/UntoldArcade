...
//  dynamic spheres (the ball) against three kinds of static geometry:
//    - world planes fed from ARKit plane detection (real floors/walls/tables),
//    - static box and sphere colliders from engine entities (the backboard,
//      the pole, and the rim's ring of sphere colliders),
...
public struct CoolBasketWorldPlane: Sendable {
    ...
    /// Multiplies the ball's restitution on this surface — a net (~0.1)
    /// kills the bounce and catches the ball.
    public var restitutionScale: Float
    /// ARKit classified this surface as the floor: the floor estimate
    /// prefers classified floors over the lowest-plane heuristic.
...
public final class CoolBasketPhysicsBackend: PhysicsBackend, @unchecked Sendable {
    ...
    private struct Body {
        ...
        /// moving hand imparts momentum on the ball (the swat).
        ...
        /// Dynamic bodies: gripped by a net pocket — integration paused
        /// until a hand touches it again.
        var isAsleep = false
        var radius: Float {
    ...
    /// Teleports a dynamic body to `position` with `velocity` — the ball
    /// reset. Removing and re-adding the body's components within one frame
    /// is invisible to the coordinator's per-substep diff, so a reset has to
    /// come through this plugin-owned channel. Returns false when the entity
    /// is not a simulated dynamic body right now (e.g. held in the hand).
    @discardableResult
    public func resetBody(
        entity: EntityID, position: SIMD3<Float>, velocity: SIMD3<Float>
    ) -> Bool {
        lock.lock()
        defer { lock.unlock() }
        guard var body = dynamicBodies[entity] else { return false }
    ...
    public func step(deltaTime: Float) {
        ...
        for (entity, var body) in dynamicBodies {
            ...
            if body.isAsleep {
                // A sleeping ball still reacts to the hands — any such
                // contact wakes it.
            ...
            // Static colliders: pole and backboard boxes, rim spheres.
            ...
            // swat. Impulse is resolved against the hand's motion so a fast
    ...
    private func resolveSphereVsStatic(
        ...
        // Geometric mean when the static collider specifies a restitution, so
        // the pole, backboard and rim each bounce their own way instead of
        // every static contact taking the ball's value.
        let staticRestitution = staticBody.descriptor.collider.restitution
        let combinedRestitution = staticRestitution > 0
            ? (restitution * staticRestitution).squareRoot()
            : restitution
    ...
    private func resolveSphereVsKinematic(
        ...
        // Snappier than a wall bounce: a swat should feel like a strike.
